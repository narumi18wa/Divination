...
local function onTouch(event)
	if (event.phase == "ended")then
        ...
        composer.removeScene("divination_3", false)
		return true
	end
end
...
function scene:create( event )
	...
	--画像表示
	usual_image = display.newImage("usual.png", _W/2, _H/4)
	usual_image:scale(0.2, 0.2)
	
	--テキスト表示
	text = display.newText("今日のあなたの運勢は不調！予期せぬ腹痛に見舞われてトイレにこもるでしょう",
...
function scene:show( event )
    ...
    elseif phase == "did" then
        ...
        usual_image:addEventListener("touch", onTouch)
