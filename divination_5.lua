...
local function onTouch(event)
	if (event.phase == "ended")then
		...
		composer.removeScene("divination_5", false)
...
function scene:create( event )
	...
	--画像表示
	bad_image = display.newImage("bad.png", _W/2, _H/4)
	bad_image:scale(0.2, 0.2)
	
	--テキスト表示
	text = 
		display.newText("今日のあなたの運勢は絶不調！！絶対外に出歩かないようにしてください。もし出歩いた場合は命の保証はありません",
...
function scene:show( event )
    ...
    elseif phase == "did" then
        ...
        bad_image:addEventListener("touch", onTouch)
